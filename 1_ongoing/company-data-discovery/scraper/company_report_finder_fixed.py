...
class FinancialReportFinder:
    ...
    def check_url_with_extended_retry(self, url):
        """
        Check if a URL exists with extended retry logic for rate limiting
        Returns: (status_code, response_object)
        """
        max_retries = 2  # Maximum number of one-hour waits
        retry_count = 0
        
        while retry_count <= max_retries:
            try:
                # First try HEAD request
                print(f"Sending HEAD request to: {url}")
                head_response = requests.head(url, headers=self.headers, timeout=self.request_timeout)
                
                # Handle 202 status code (request accepted but processing)
                if head_response.status_code == 202:
                    print(f"[RATE LIMIT] Server returned status code 202 - likely rate limited")
                    
                    # First try with short pauses
                    short_retry_success = False
                    print("Attempting short retries first...")
                    for attempt in range(3):
                        wait_time = 10  # 10 seconds between short retries
                        print(f"Waiting {wait_time} seconds before short retry (attempt {attempt+1}/3)...")
                        time.sleep(wait_time)
                        
                        head_response = requests.head(url, headers=self.headers, timeout=self.request_timeout)
                        if head_response.status_code == 200:
                            print("[SUCCESS] Short retry succeeded, received status code 200")
                            return head_response.status_code, head_response
                        print(f"[FAILED] Still getting status code {head_response.status_code}")
                    
                    # If short retries didn't work, try with GET request
                    print(f"[STRATEGY] Trying GET request instead of HEAD")
                    try:
                        get_response = requests.get(url, headers=self.headers, timeout=self.request_timeout, stream=True)
                        # Close the connection immediately after checking status
                        get_response.close()
                        if get_response.status_code == 200:
                            print("[SUCCESS] GET request successful")
                            head_response.status_code = 200  # Use the HEAD response object but update status
                            return 200, head_response
                    except Exception as get_e:
                        print(f"[ERROR] GET request failed: {str(get_e)}")
                    
                    # If we're still here, both HEAD and GET failed, implement the long one-hour wait
                    if retry_count < max_retries:
                        wait_time = 60 * 60  # 1 hour in seconds
                        retry_count += 1
                        
                        print(f"=" * 80)
                        print(f"[RATE LIMIT] Both HEAD and GET retries failed. Implementing long wait strategy.")
                        print(f"[RATE LIMIT] Waiting for 1 hour before retry {retry_count}/{max_retries}")
                        print(f"[RATE LIMIT] Current time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
                        print(f"[RATE LIMIT] Will retry at: {(datetime.now() + pd.Timedelta(seconds=wait_time)).strftime('%Y-%m-%d %H:%M:%S')}")
                        print(f"=" * 80)
                        
                        # Implement a countdown to provide feedback during the wait
                        countdown_interval = 5 * 60  # Show message every 5 minutes
                        for remaining in range(wait_time, 0, -countdown_interval):
                            minutes_remaining = remaining // 60
                            print(f"[RATE LIMIT] {minutes_remaining} minutes remaining until next retry...")
                            time.sleep(countdown_interval)
                            
                        print(f"[RATE LIMIT] Wait complete. Retrying URL check for: {url}")
                        continue  # Go back to the start of the while loop
                    else:
                        print(f"[RATE LIMIT] Maximum retries reached. Unable to check URL: {url}")
                        return 404, None  # Return 404 as a fallback
                
                # If HEAD request fails with non-202 status, try GET as fallback
                if head_response.status_code != 200:
                    print(f"[STRATEGY] HEAD request failed with status {head_response.status_code}, trying GET request...")
                    try:
                        get_response = requests.get(url, headers=self.headers, timeout=self.request_timeout, stream=True)
                        # Close the connection immediately after checking status
                        get_response.close()
                        if get_response.status_code == 200:
                            print("[SUCCESS] GET request successful")
                            return 200, head_response  # We return the HEAD response object but with 200 status
                    except Exception as get_e:
                        print(f"[ERROR] GET request also failed: {str(get_e)}")
                
                # If we get here, return what we have
                return head_response.status_code, head_response
            
            except Exception as e:
                print(f"[ERROR] Error checking URL {url}: {str(e)}")
                import traceback
                traceback.print_exc()
                return 404, None  # Return 404 as a fallback
    ...
    def search_duckduckgo(self, query):
        ...
        try:
            ...
            while retry_count <= max_retries:
                print(f"Sending search request to DuckDuckGo for: {query}")
                response = requests.get(search_url, headers=self.headers, timeout=self.request_timeout)
                
                # Handle 202 status code (request accepted but processing)
                if response.status_code == 202:
                    print(f"[RATE LIMIT] DuckDuckGo returned status code 202 - likely rate limited")
                    ...
                    for attempt in range(3):
                        ...
                        print(f"Retrying search request for: {query}")
                        response = requests.get(search_url, headers=self.headers, timeout=self.request_timeout)
                        if response.status_code == 200:
                            print("[SUCCESS] Short retry succeeded, received status code 200")
                            short_retry_success = True
                            break
                        print(f"[FAILED] Still getting status code {response.status_code}")
                    
                    # If short retries didn't work, implement the long one-hour wait
                    if not short_retry_success:
                        if retry_count < max_retries:
                            wait_time = 60 * 60  # 1 hour in seconds
                            retry_count += 1
                            
                            print(f"=" * 80)
                            print(f"[RATE LIMIT] Short retries failed. Implementing long wait strategy.")
                            print(f"[RATE LIMIT] Waiting for 1 hour before retry {retry_count}/{max_retries}")
                            print(f"[RATE LIMIT] Current time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
                            print(f"[RATE LIMIT] Will retry at: {(datetime.now() + pd.Timedelta(seconds=wait_time)).strftime('%Y-%m-%d %H:%M:%S')}")
                            print(f"=" * 80)
                            ...
                            print(f"[RATE LIMIT] Wait complete. Retrying search request for: {query}")
                            continue  # Go back to the start of the while loop
                        else:
                            print(f"[RATE LIMIT] Maximum retries reached. Unable to get successful response from DuckDuckGo.")
                            return []
                
                # If we get here, we either got a successful response or some other error
                if response.status_code != 200:
                    print(f"[ERROR] Search failed with status code: {response.status_code}")
                    return []
                
                # If we got a 200 response, process it
                print(f"[SUCCESS] Search request successful. Processing results...")
                ...
                if not result_elements:
                    print(f"[WARNING] No results found in the response. DuckDuckGo may have changed their HTML structure.")
                ...
                for result in result_elements:
                    ...
                    if any(domain in actual_url.lower() for domain in self.non_european_domains):
                        print(f"[FILTER] Skipping non-European domain: {actual_url}")
                        continue
                ...
                print(f"[SUCCESS] Found {len(results)} results after filtering")
                return results
                
        except Exception as e:
            print(f"[ERROR] Error in search: {str(e)}")
            import traceback
            traceback.print_exc()
            return []
        ...
        try:
            ...
            if not pdf_links:
                ...
                for pattern_url in common_patterns:
                    try:
                        # Check if URL exists using our extended retry method
                        status_code, _ = self.check_url_with_extended_retry(pattern_url)
                        ...
                        if status_code == 200:
                            ...
                            print(f"[SUCCESS] Found common pattern match: {pattern_url}")
                    except Exception as e:
                        print(f"[ERROR] Error checking pattern URL {pattern_url}: {str(e)}")
                        continue
            ...
            # SNCF pattern
            elif 'sncf.com' in domain or 'groupe-sncf.com' in domain:
                company_patterns = [
                    "https://www.groupe-sncf.com/medias-publics/2024-03/sncf-group-annual-financial-report-2023.pdf",
                    "https://medias.sncf.com/sncfcom/finances/Publications_financieres/SNCF_RAA_31122022_EN.pdf",
                    "https://medias.sncf.com/sncfcom/finances/Publications_financieres/SNCF_RAA_31122021_EN.pdf"
                ]
            ...
            for pattern_url in company_patterns:
                try:
                    # Check if URL exists using our extended retry method
                    status_code, head_response = self.check_url_with_extended_retry(pattern_url)
                    ...
                    if status_code == 200:
                        ...
                        print(f"[SUCCESS] Found valid PDF: {pattern_url}")
                except Exception as e:
                    print(f"[ERROR] Error checking URL {pattern_url}: {str(e)}")
                    continue
    ...
    def find_company_reports(self, company_name):
        ...
        # Add company-specific search queries for certain companies
        if company_name == "FCC":
            search_queries = [
        ...
        elif "SNCF" in company_name:
            search_queries = [
                "SNCF annual financial report pdf",
                "SNCF groupe railway annual report",
                "Société Nationale des Chemins de fer Français financial report",
                "SNCF financial statements french railways"
            ]
        ...
        for query in search_queries[:2]:  # Try first 2 queries initially
            ...
            # Check if any are direct PDFs
            for page in pages:
                if page.get('is_direct_pdf', False):
                    direct_pdf_results.append({
                        'url': page['url'],
                        'text': page['title'],
                        'year': page.get('year', 'Unknown'),
                        'score': page.get('score', 10)  # High score for direct PDFs
                    })
        ...
        # If we found direct PDFs, process them first
        verified_pdfs = []
        if direct_pdf_results:
            print(f"[DIRECT PDF] Found {len(direct_pdf_results)} direct PDF links in search results")
            # Sort by year (most recent first)
            sorted_direct_pdfs = sorted(
                direct_pdf_results,
                key=lambda x: (int(x['year']) if x['year'].isdigit() else 0),
                reverse=True
            )
            
            # Verify that PDFs are accessible
            for pdf in sorted_direct_pdfs:
                status_code, _ = self.check_url_with_extended_retry(pdf['url'])
                if status_code == 200:
                    verified_pdfs.append(pdf)
                    print(f"[DIRECT PDF] Verified accessible PDF: {pdf['url']}")
        ...
        # Process any direct PDFs found in the second round
        if direct_pdf_results and not verified_pdfs:
            print(f"[DIRECT PDF] Found {len(direct_pdf_results)} direct PDF links in additional search results")
            # Sort by year (most recent first)
            sorted_direct_pdfs = sorted(
                direct_pdf_results,
                key=lambda x: (int(x['year']) if x['year'].isdigit() else 0),
        ...
        # Add special case for SNCF (since you mentioned it was a problem)
        if "SNCF" in company_name.upper():
            special_urls = [
                "https://www.groupe-sncf.com/medias-publics/2024-03/sncf-group-annual-financial-report-2023.pdf",
                "https://medias.sncf.com/sncfcom/finances/Publications_financieres/SNCF_RAA_31122022_EN.pdf"
            ]
            ...
            for url in special_urls:
                print(f"[SPECIAL CASE] Checking SNCF special URL: {url}")
                status_code, _ = self.check_url_with_extended_retry(url)
                if status_code == 200:
                    ...
                    direct_result = [{
                        'url': url,
                        'text': f"SNCF Annual Financial Report {year}",
                        'year': year,
                        'score': 15  # Very high score for known URL
                    }]
                    print(f"[SPECIAL CASE] Found valid SNCF PDF: {url}")
    ...
    def try_known_company_patterns(self, company_name):
        ...
        if matched_company:
            print(f"[MATCH] Found matched company pattern: {matched_company}")
            for pattern_url in company_patterns[matched_company]:
                try:
                    ...
                    if status_code == 200:
                        ...
                        print(f"[SUCCESS] Found direct match: {pattern_url}")
                except Exception as e:
                    print(f"[ERROR] Error checking pattern URL {pattern_url}: {str(e)}")
                    continue
